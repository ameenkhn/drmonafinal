...
function initHeaderSticky() {
  var windowOn = $(window);
  var body = $('body');
  var isMonaBody = body.hasClass('mona-body');
  var headerTarget = isMonaBody ? $('.mona-header') : $('.vl-header-area');

  function updateHeaderState() {
    var scroll = windowOn.scrollTop();
    if (isMonaBody) {
      headerTarget.removeClass("header-sticky");
      if (scroll < 10) {
        headerTarget.removeClass("is-scrolled");
      } else {
        headerTarget.addClass("is-scrolled");
      }
    } else {
      if (scroll < 100) {
        headerTarget.removeClass("header-sticky");
      } else {
        headerTarget.addClass("header-sticky");
      }
    }
  }

  windowOn.on('scroll', updateHeaderState);
  updateHeaderState();
}
initHeaderSticky();
//========== HEADER ACTIVE ENDS ============= //
